...
export function computeBills(shipping: number, items: Array<Shipping>) {
  ...
  const grandTotal: number = items
    .map((item) => computeTotal(item.quantity, item.price))
    .reduce((a, b) => a + b, 0);
...
function buildMap(items: Shipping[]) {
  ...
  items.forEach((item) => {
    ...
    if (totals.has(item.name)) {
      totals.set(
        item.name,
        totals.get(item.name)! + computeTotal(item.quantity, item.price)
      );
    }
  });
