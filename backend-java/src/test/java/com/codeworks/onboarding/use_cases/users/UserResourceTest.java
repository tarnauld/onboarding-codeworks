...
public class UserResourceTest {
    ...
    private List<UserEntity> buildUsers() {
        return Arrays.asList(
                UserEntity.builder().id(1L).name("John").birthDate(LocalDate.now()).build(),
                UserEntity.builder().id(2L).name("Clara").birthDate(LocalDate.now()).build(),
                UserEntity.builder().id(3L).name("Bertrand").birthDate(LocalDate.now()).build(),
                UserEntity.builder().id(4L).name("Alice").birthDate(LocalDate.now()).build()
        );
    }
}
